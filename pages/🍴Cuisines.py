...
selected_cuisines = st.sidebar.multiselect('Select cuisines', df['cuisines'].unique(),default = df['cuisines'].value_counts().nlargest(5).index.tolist())
selected_countries = st.sidebar.multiselect('Select countries', df['country'].unique(),default = df['country'].value_counts().nlargest(5).index.tolist())
